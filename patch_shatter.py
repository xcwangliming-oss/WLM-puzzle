...
old_tl_to = """      blocksToRemove.forEach(b => {

        tl.to(b.sprite.scale, { x: 0, y: 0, duration: 0.3, ease: 'back.in(1.5)' }, 0);

      });"""

new_tl_to = """      blocksToRemove.forEach(b => {
        tl.to(b.sprite.scale, { x: 0, y: 0, duration: 0.2, ease: 'power2.in' }, 0);
...
if old_tl_to in ts:
    ...
    print("Error: Could not find blocksToRemove loop to patch.")
    sys.exit(1)
