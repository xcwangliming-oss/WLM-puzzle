...
assert.match(
  mainSource,
  /function sendSheepToPastureBlock[\s\S]*?gsap\.to\(visitor, \{[\s\S]*?x: targetX[\s\S]*?duration: 1[\s\S]*?ease: 'none'[\s\S]*?onComplete: completeArrival/,
  'sheep visitor must animate right-to-left inside the fixed grass-block mask at the requested one-second linear speed',
);
assert.match(
  mainSource,
