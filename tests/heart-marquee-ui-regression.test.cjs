...
assert.match(html, /#board-wrapper\.heart-top-ui-live #board-clip \{[\s\S]*?background:\s*transparent !important;[\s\S]*?border-radius:\s*0 !important;[\s\S]*?box-shadow:\s*none !important;/, 'heart top UI should remove the solid board clip frame under the grid');
