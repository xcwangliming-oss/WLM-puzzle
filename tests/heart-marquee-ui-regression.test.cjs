...
assert.match(source, /comboCount \+= comboIncrement;[\s\S]*?triggerHeartClearHud\(\);/, 'eliminations should trigger the heart clear animation');
...
assert.match(source, /comboCount \+= comboIncrement;[\s\S]*?triggerHeartClearHud\(\);[\s\S]*?triggerMarqueeClearEffect\(\);/, 'eliminations should trigger the marquee clear animation alongside the heart HUD');
