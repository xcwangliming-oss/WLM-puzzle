...
assert.match(mainSource, /const ROW_CLEAR_HIGHLIGHT_SECONDS = 0\.18;[\s\S]*?function scheduleRowClearHighlight\([\s\S]*?ColorMatrixFilter[\s\S]*?brightness\(1\.75, false\)[\s\S]*?block\.sprite\.filters = originalFilters\.get\(block\.id\) \|\| \[\]/, 'full rows should briefly highlight before clearing');
...
assert.match(mainSource, /tl\.call\(\(\) => \{\s*playTntSound\(\);[\s\S]*?playTntPreExplosionEffect\(block, TNT_PRE_EXPLOSION_SECONDS\);/, 'TNT sound should start when the pre-explosion shake begins');
assert.doesNotMatch(mainSource, /tl\.call\(\(\) => \{[\s\S]*?sprite\.zIndex = 20004;[\s\S]*?playTntSound\(\);[\s\S]*?playTntBurstWave\(block\);[\s\S]*?playTntExplosionEffect\(block\);[\s\S]*?\}, \[\], at \+ TNT_PRE_EXPLOSION_SECONDS\);/, 'TNT sound should not wait for the block shatter phase');
...
assert.match(mainSource, /const blastAt = rowClearHighlightDelay \+ \(detonatingTntIds\.size > 0 \? \(tntBlastRemovalTimes\.get\(block\.id\) \?\? TNT_PRE_EXPLOSION_SECONDS\) : 0\);/, 'TNT blast removals should wait for the row highlight and relevant TNT explosion time');
...
assert.match(mainSource, /const rowClearHighlightDelay = fullRows\.length > 0 \? ROW_CLEAR_HIGHLIGHT_SECONDS : 0;[\s\S]*?scheduleRowClearHighlight\(tl, blocksToRemove, rowClearHighlightDelay\);[\s\S]*?const rowPlaybackOffset = rowClearHighlightDelay \+ rowPlaybackIndex \* rowPlaybackGap;[\s\S]*?scheduleTntDetonation\(tl, b, rowClearHighlightDelay \+ \(tntDetonationStartTimes\.get\(b\.id\) \?\? 0\)\)/, 'row clears should highlight before regular shatter or TNT detonation starts');
