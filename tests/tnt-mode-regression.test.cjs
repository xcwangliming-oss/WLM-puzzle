...
assert.match(mainSource, /const shouldSyncTntThreeRowShatter = tntBlastMode === 'three-rows' && detonatingTntIds\.size > 0;[\s\S]*?const tntRowShatterTimes = new Map<number, number>\(\);[\s\S]*?if \(shouldSyncTntThreeRowShatter\)[\s\S]*?tntBlastRemovalTimes\.get\(block\.id\)[\s\S]*?tntRowShatterTimes\.set\(block\.row[\s\S]*?tntRowShatterTimes\.forEach\(\(blastAt, row\)[\s\S]*?const shatterCols = new Set<number>\(\);[\s\S]*?shatterCols\.add\(block\.col \+ offset\);[\s\S]*?if \(shatterCols\.size === 0\) return;[\s\S]*?playRowShatterEffect\(row, explosionColor, rowBlocks, propSkipCols, shatterCols, true\)/, 'three-row TNT mode should only play shatter on occupied affected cells while preserving row shatter timing');
