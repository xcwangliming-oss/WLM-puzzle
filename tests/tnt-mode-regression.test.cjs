...
assert.match(mainSource, /function playTntSound\(\)[\s\S]*?cloneNode\(true\)[\s\S]*?audio\.play\(\)/, 'TNT sound should use a cloned audio instance so chained explosions do not cut each other off');
...
assert.match(mainSource, /const tntRowShatterTimes = new Map<number, number>\(\);[\s\S]*?tntBlastMode === 'three-rows'[\s\S]*?tntBlastRemovalTimes\.get\(block\.id\)[\s\S]*?tntRowShatterTimes\.set\(block\.row[\s\S]*?tntRowShatterTimes\.forEach\(\(blastAt, row\)[\s\S]*?playRowShatterEffect\(row, explosionColor, rowBlocks, propSkipCols\)/, 'three-row TNT mode should play all affected row shatters at the TNT blast time');
assert.match(mainSource, /if \(!tntRowShatterTimes\.has\(r\) && PARAMS\.effectType !== 'gem-shatter'\)[\s\S]*?const tntRowShatterAt = tntRowShatterTimes\.get\(b\.row\);[\s\S]*?const visualDelay = tntRowShatterAt !== undefined[\s\S]*?\? tntRowShatterAt/, 'ordinary rows should keep old shatter timing while TNT-hit rows wait and break together');
