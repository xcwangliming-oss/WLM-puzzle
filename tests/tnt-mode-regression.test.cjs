...
assert.match(mainSource, /function applyRowClearHighlight\(rowBlocks: Block\[\]\)[\s\S]*?ColorMatrixFilter[\s\S]*?brightness\(1\.75, false\)[\s\S]*?block\.sprite\.filters = \[brightFilter\]/, 'clearing blocks should highlight and stay highlighted until clearing');
...
assert.match(mainSource, /const blastAt = detonatingTntIds\.size > 0 \? \(tntBlastRemovalTimes\.get\(block\.id\) \?\? TNT_PRE_EXPLOSION_SECONDS\) : 0;/, 'TNT blast removals should wait for the relevant TNT explosion time without extra highlight delay');
...
assert.match(mainSource, /tl\.call\(\(\) => \{\s*applyRowClearHighlight\(blocksToRemove\);\s*\}, \[\], 0\);[\s\S]*?const rowPlaybackOffset = rowPlaybackIndex \* rowPlaybackGap;[\s\S]*?scheduleTntDetonation\(tl, b, tntDetonationStartTimes\.get\(b\.id\) \?\? 0\)/, 'completed rows should highlight immediately without delaying regular shatter or TNT detonation');
assert.doesNotMatch(mainSource, /rowClearHighlightDelay|ROW_CLEAR_HIGHLIGHT_SECONDS/, 'row highlight should not add a separate delay before clearing');
