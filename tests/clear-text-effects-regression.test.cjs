...
assert.match(source, /const blocksToRemove = lockedSequentialIdSet[\s\S]*?const comboIncrement = Math\.max\(1, tntThreeRowComboRows \|\| fullRows\.length\);[\s\S]*?comboCount \+= comboIncrement;[\s\S]*?rowsForPlayback\.forEach\(\(r, rowPlaybackIndex\) => \{[\s\S]*?const rowBlocks = blocksToRemove\.filter\(b => b\.row === r\);[\s\S]*?tl\.call\(\(\) => \{[\s\S]*?if \(rowPlaybackIndex === 0\) \{[\s\S]*?triggerComboTextEffect\(fullRows, comboCount, rowBlocks\.length > 0 \? rowBlocks : blocksToRemove\);/, 'multi-row eliminations should add the cleared row count, TNT three-row blasts should add blasted rows, and trigger one combo at the first actual shatter row');
