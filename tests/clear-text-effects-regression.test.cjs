...
assert.match(source, /function triggerComboTextEffect\(rows: number\[\], combo: number, anchorBlocks: Block\[\] = \[\]\)[\s\S]*?const comboValue = Math\.max\(1, combo\);[\s\S]*?const comboWordUrl = getComboWordUrl\(comboValue\);[\s\S]*?const digitUrls = getComboDigitUrls\(comboValue\);[\s\S]*?pushClearTextEffect\(\{ type: 'combo'[\s\S]*?comboCount: comboValue \}\);[\s\S]*?appendComboTextEffectAtPoint/, 'combo text should create one effect per elimination wave and show the continuous elimination combo count');
assert.match(source, /type ClearTextEffect = \{[\s\S]*?boardX\?: number;[\s\S]*?boardY\?: number;[\s\S]*?boardYRatio\?: number;/, 'combo text effects should remember their trigger-time board position');
assert.match(source, /function getBoardBlocksEffectPoint\(anchorBlocks: Block\[\], offsetPx: number\)[\s\S]*?bottomRowTopGameY = Math\.max\(\.\.\.anchorBlocks\.map\(block => block\.sprite\?\.y \?\? block\.row \* PARAMS\.cellSize\)\)[\s\S]*?rowTop - offsetPx/, 'combo text should anchor to the bottom-most actual removed row during autoplay');
assert.match(source, /const edgeSafeY = Math\.max\(72, offsetPx\);[\s\S]*?const minY = clipTop \+ edgeSafeY;[\s\S]*?const maxY = clipBottom - edgeSafeY;[\s\S]*?const y = Math\.max\(minY, Math\.min\(maxY, rowTop - offsetPx\)\);/, 'live combo row mapping should anchor above the cleared row while keeping the text inside the board');
assert.match(source, /getBoardRowEffectPoint\(targetRow, 30\)/, 'combo text should fall back to 30px above the cleared row when removed blocks are unavailable');
assert.match(source, /const point = getBoardBlocksEffectPoint\(anchorBlocks, 30\) \|\| getBoardRowEffectPoint\(targetRow, 30\);/, 'combo text should prefer actual removed blocks over recorded row numbers');
assert.match(source, /pushClearTextEffect\(\{ type: 'combo'[\s\S]*?boardX: point\.x[\s\S]*?boardY: point\.y[\s\S]*?boardYRatio: point\.yRatio[\s\S]*?comboCount: comboValue \}\)/, 'combo text should store the fixed trigger position before recording redraws');
assert.match(source, /function getBoardRowEffectPoint\(row: number, offsetPx: number\)[\s\S]*?const visibleHeight = Math\.max\(1, getViewportGameHeight\(\)\);/, 'live combo row mapping should use the visible board height instead of the full generated board height');
...
assert.match(source, /comboCount \+= Math\.max\(1, fullRows\.length\);[\s\S]*?const blocksToRemove = lockedSequentialIdSet[\s\S]*?rowsForPlayback\.forEach\(\(r, rowPlaybackIndex\) => \{[\s\S]*?const rowBlocks = blocksToRemove\.filter\(b => b\.row === r\);[\s\S]*?const effectRowBlocks = rowBlocks\.filter\(b => !b\.pastureStage\);[\s\S]*?tl\.call\(\(\) => \{[\s\S]*?if \(rowPlaybackIndex === 0\) \{[\s\S]*?const comboAnchorBlocks = effectRowBlocks\.length > 0[\s\S]*?triggerComboTextEffect\(fullRows, comboCount, comboAnchorBlocks\.length > 0 \? comboAnchorBlocks : blocksToRemove\);/, 'multi-row eliminations should add the cleared row count and trigger one combo at the first actual shatter row');
...
assert.match(source, /const mappedTriggerPoint = wrapperRect && recordingWrapperBox && typeof effect\.boardX === 'number' && typeof effect\.boardY === 'number'[\s\S]*?mapBoardWrapperRectToRecordingRect\([\s\S]*?new DOMRect\(wrapperRect\.left \+ effect\.boardX, wrapperRect\.top \+ effect\.boardY, 1, 1\)[\s\S]*?const rowY = mappedTriggerPoint[\s\S]*?\? mappedTriggerPoint\.y[\s\S]*?: fixedBoardYRatio !== null/, 'recording combo text should map the saved live trigger point into the recording canvas before falling back to ratios');
assert.match(source, /function drawRecordingClearTextEffects\([\s\S]*?const visibleHeight = Math\.max\(1, getViewportGameHeight\(\)\);/, 'recording combo row mapping should use the visible board height instead of the full generated board height');
...
assert.match(source, /drawRecordingMarqueeBorder\(recordingCtx!, boardClipBox, performance\.now\(\)\);[\s\S]*?drawRecordingClearTextEffects\(recordingCtx!, boardClipBox, performance\.now\(\), \{ x: 0, y: 0, w: width, h: height \}\);/, 'recording should draw clear text over the board after the marquee border using the full wrapper mapping');
