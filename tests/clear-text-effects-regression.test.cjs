...
assert.match(source, /comboCount \+= Math\.max\(1, fullRows\.length\);[\s\S]*?const blocksToRemove = lockedSequentialIdSet[\s\S]*?rowsForPlayback\.forEach\(\(r, rowPlaybackIndex\) => \{[\s\S]*?const rowBlocks = blocksToRemove\.filter\(b => b\.row === r\);[\s\S]*?tl\.call\(\(\) => \{[\s\S]*?if \(rowPlaybackIndex === 0\) \{[\s\S]*?triggerComboTextEffect\(fullRows, comboCount, rowBlocks\.length > 0 \? rowBlocks : blocksToRemove\);/, 'multi-row eliminations should add the cleared row count and trigger one combo at the first actual shatter row');
