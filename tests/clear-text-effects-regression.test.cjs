...
assert.match(source, /const minY = clipTop \+ 30;[\s\S]*?const maxY = clipBottom - 30;[\s\S]*?const y = Math\.max\(minY, Math\.min\(maxY, rowTop - offsetPx\)\);/, 'live combo row mapping should anchor 30px above the cleared row while staying visible in the board');
