...
assert.match(source, /function getBoardBlocksEffectPoint\(anchorBlocks: Block\[\], offsetPx: number\)[\s\S]*?topGameY = Math\.min\(\.\.\.anchorBlocks\.map\(block => block\.sprite\?\.y \?\? block\.row \* PARAMS\.cellSize\)\)[\s\S]*?rowTop - offsetPx/, 'combo text should anchor to the actual currently removed block sprites during autoplay');
