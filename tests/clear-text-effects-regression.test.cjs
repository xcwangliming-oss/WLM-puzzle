...
assert.match(source, /comboCount \+= Math\.max\(1, fullRows\.length\);[\s\S]*?const blocksToRemove = lockedSequentialIdSet[\s\S]*?triggerComboTextEffect\(fullRows, comboCount, blocksToRemove\);/, 'multi-row eliminations should add the cleared row count and anchor combo text to removed blocks');
