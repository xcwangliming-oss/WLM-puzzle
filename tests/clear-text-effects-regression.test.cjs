...
assert.match(source, /type ClearTextEffect = \{[\s\S]*?boardY\?: number;[\s\S]*?boardYRatio\?: number;/, 'combo text effects should remember their trigger-time board position');
...
assert.match(source, /pushClearTextEffect\(\{ type: 'combo'[\s\S]*?boardY: point\.y[\s\S]*?boardYRatio: point\.yRatio[\s\S]*?comboCount: comboValue \}\)/, 'combo text should store the fixed trigger position before recording redraws');
...
assert.match(source, /const fixedBoardYRatio = typeof effect\.boardYRatio === 'number'[\s\S]*?boardBox\.y \+ boardBox\.h \* fixedBoardYRatio[\s\S]*?: boardBox\.y \+ \(\(effect\.row \* PARAMS\.cellSize\) \+ \(worldContainer\?\.y \|\| 0\)\) \* scaleY - 30/, 'recording combo text should prefer the fixed trigger position instead of drifting with later board scroll');
