...
assert.match(source, /function appendComboTextEffectAtPoint\([\s\S]*?const wordWidth = Math\.min\(178, boardWidth \* 0\.26\);[\s\S]*?const digitWidth = Math\.min\(41, boardWidth \* 0\.065\);[\s\S]*?const digitHeight = digitWidth \* 1\.24;[\s\S]*?const digitGap = digitWidth \* 1\.05;[\s\S]*?appendClearTextImage\('combo-word'[\s\S]*?appendClearTextImage\('combo-digit'[\s\S]*?160, digitWidth, digitHeight/, 'combo text should render digits in a fixed-size contain box with non-overlapping delayed digits');
